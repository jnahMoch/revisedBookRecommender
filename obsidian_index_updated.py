"""
BookRecommender Frontend with Semantic Search Integration
Gradio UI with text search, filtering, and semantic retrieval.
"""
...
print(f"[INIT] Loaded dataset: {SOURCE_FILE}")
print(f"[INIT] Initializing semantic search...")
...
if not BOOKS_DF.empty:
    try:
        ...
        for idx, row in BOOKS_DF.iterrows():
            # Create a rich document combining title, genre, and description
            doc_text = f"{clean_text(row.get('title', ''))} {clean_text(row.get('genre', ''))} {clean_text(row.get('description', '')[:500])}"
        ...
        print("[INIT] ✓ Semantic index ready")
    except Exception as e:
        print(f"[INIT] ⚠ Semantic search initialization failed: {e}")
        CHROMA_DB = None
...
def rank_books(query, collection, genre, mood, minimum_rating):
    """Rank books based on multiple criteria including semantic similarity."""
    if BOOKS_DF.empty:
        return BOOKS_DF
    ...
    # Parse query for text terms and emotions
    ...
    # Semantic search if query exists
    if query and CHROMA_DB:
        semantic_indices = semantic_search_query(query, k=len(df))
        if semantic_indices:
            # Give semantic matches a boost
            for idx in semantic_indices[:min(20, len(semantic_indices))]:
                if idx in df.index:
                    score.loc[idx] += 5.0  # High semantic score boost
    ...
    # Text search on remaining dataset
